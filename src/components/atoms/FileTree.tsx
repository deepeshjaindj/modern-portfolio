...
import { LuFileCode, LuFolderClosed, LuFolderOpen } from "react-icons/lu";
...
const Tree = forwardRef<HTMLDivElement, TreeViewProps>(
  (
    ...
    return (
      ...
      >
        <div className={classnames("size-full", className)}>
          <div
            ref={ref}
      ...
      >
        ...
        >
          {expandedItems?.includes(value)
            ? (openIcon ?? <LuFolderOpen className="size-4" />)
            : (closeIcon ?? <LuFolderClosed className="size-4" />)}
          <span>{element}</span>
        </AccordionPrimitive.Trigger>
  ...
  (
    ...
    return (
      <button
        ...
        {...props}
      >
        {fileIcon ?? <LuFileCode className="size-4" />}
        {children}
      </button>
    );
  },
